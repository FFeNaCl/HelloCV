...
class ArmorNode : public rclcpp::Node {
  ...
  ArmorNode(string name) : Node(name) {
    RCLCPP_INFO(this->get_logger(), "Initializing ArmorNode");//输出信息1
...
int main(int argc, char **argv) {
  ...
  RCLCPP_INFO(node->get_logger(), "Starting ArmorNode");//输出信息3
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
...
void ArmorNode::callback_camera(sensor_msgs::msg::Image::SharedPtr msg) {
  try {
    ...
    vector<string> types = {"armor_red_1"};
    ...
    RCLCPP_INFO(this->get_logger(), "Published %d armor_red_1 targets",msg_object.num_objects);
